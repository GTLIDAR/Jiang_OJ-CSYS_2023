...
class SparseGaussianProcessYaw:
  ...
  def __init__(self):
    # Initialize variance such that all known g(x) are within one standard deviation
    self.variance = (meanYawError+noiseStd) ** 2
    # Initialize length
    self.l = [0.1,5]
    # Initialize Data Points
    self.pos = np.empty((2,0))
  ...
  def append(self,pos,val):
    ...
    self.pos = pos
    # Append g(x) value
    self.val = abs(val) - meanYawError - noiseStd

  def calcMean(self,pos):
    # edge case with no samples
    if self.pos.shape[1] == 0:
      return np.array([[0]]) + meanYawError + noiseStd
    ...
    return f_test + meanYawError + noiseStd
...
# Kernel hyperparameters (length,standard deviation)
theta_fixed_Y = jnp.array([2.5,90,meanYawError+noiseStd])
...
# Initialize Plotting
# img = plt.imread("legged_robot_simple_casestudy.png")
# f3 = plt.figure()
...
frame_count = 0
compression = 20
# prev_feet = plt.scatter([], [], s=10, c="lightsalmon", alpha = 0.4, marker = 'o')
# curr_feet = plt.scatter([], [], s=10, c="darkred", alpha = 0.4, marker = 'o')
# prev_com = plt.scatter([], [], s=10, c="black",alpha=0.8, marker='o')
# curr_com = plt.scatter([], [], s=10, c="cornflowerblue", alpha=0.8, marker = 'o')
...
class SimpleGoal:
  ...
  def __init__(self):
    ...
    self.currentPos = np.array([[stateLength/2],[stateLength/2],[22.5],[0]])
    ...
    self.gT.append(self.centers, SimpleGoal.terrainE)
    self.gT.train()
    # Find current foot position
    self.footPos = np.array([[stateLength/2],[stateLength/2+.1],[self.getElevationA(np.array([[stateLength/2],[stateLength/2+.1]])).item()]])
    self.prevFoot = self.footPos
    self.prevYaw = self.currentPos[2]
    # Initialize stochastic noise
    self.noiseStd = noiseStd
    self.noiseVariance = self.noiseStd ** 2
    self.v = stats.truncnorm(-15,15,loc=0,scale=self.noiseStd)
  ...
  def step(self,state,action):
    ...
    if action == 'b':
      ...
      self.currentPos[2] = copy.deepcopy(tcen[2])
      ...
      if self.currentPos[3]==0:
        ...
        for ad in angd:
          ...
          #count = 1-count
          prevA = a
        rf[0:2] = self.currentPos[0:2]-footSpace*np.array([[math.cos(ang+self.dt+math.pi/2)],[math.sin(ang+self.dt+math.pi/2)]])
        # rf[2] = self.getElevationA(self.rf[0:2])
        #self.writeFeet(lf,rf,np.array([3]),self.currentPos[2],self.currentPos[2])
        #self.writeFeet(rf,self.currentPos[2])
        ...
        # lf[2] = self.getElevationA(self.rf[0:2])
        #self.writeFeet(lf,rf,np.array([3]),self.currentPos[2],self.currentPos[2])
        #self.writeFeet(lf,self.currentPos[2])
    ...
    ddt = 0
    #for k in range(100):
    #  ddt = 0.99*ddt-0.01*self.gpM.calcMean(np.array([[self.dd],[self.dt.item()+float(ddt)],[self.dz.item()],[self.currentPos[3].item()]]))
    self.dt = self.dt + ddt
    ...
    #gxgy = self.currentPos[0:2]+self.dd*np.array([[math.cos(self.desAng*math.pi/180)],[math.sin(self.desAng*math.pi/180)]])
    #tcen[0]=gxgy[0]
    #tcen[1]=gxgy[1]
    ...
    if action=='s':
      ...
      self.angError = np.array([self.angError])
        ...
        #self.writeFeet(self.prevFoot,self.currentPos[2]*math.pi/180)
      #else:
        #self.writeFeet(self.footPos,self.currentPos[2]*math.pi/180)
        #self.writeFeet(self.prevFoot,self.currentPos[2]*math.pi/180)
    else:
      ...
      if self.currentPos[3]==0 and check == 0:
        ...
        self.prevFoot[0:2]=self.currentPos[0:2]-footSpace*np.array([[math.cos(ang)],[math.sin(ang)]])
        self.prevFoot[2]=self.getElevationA(self.prevFoot[0:2]) 
        #self.writeFeet(self.prevFoot,self.footPos,np.array([self.t]),self.currentPos[2],self.currentPos[2])
        #self.writeFeet(self.prevFoot,self.currentPos[2]*math.pi/180)
      if check == 0:
  ...
  def makeStates(self):
    ...
    #self.hazardids=np.arange(90,91,1)
    self.hazardids = np.concatenate((np.arange(42,79,10),np.arange(55,84,20),np.arange(49,50,2)))
    #self.hazardids = np.array([55,72])
  ...
  def calcTransitionProbabilitiesFirst(self):
    ...
    for state in self.s.states:
      ...
      if id in self.hazards:
        ...
        xs = np.linspace(cen[0]-stateLength/2,cen[0]+stateLength/2,num=3)
        ys = np.linspace(cen[1]-stateLength/2,cen[1]+stateLength/2,num=3)
        zmin = 1000
        zmax = 0
        for x in xs:
          for y in ys:
  ...
  def calcTransitionProbabilities(self):
    ...
    for state in self.s.states:
      id = state.getID()
      if id in self.hazards or id in self.goals:
        continue
      else:
        #print(id)
        cen = self.getCenter(state)
  ...
  def calcUncertainty(self):
    # Loop through states
    visited = [0, 12, 24, 36, 58, 46, 44, 48, 34, 22, 1, 13, 25, 27, 39, 15, 11, 23, 35, 57, 78, 99]
    tempUncertainty = 0
    for state in self.s.states:
      if math.floor(state.getID() / 48) in visited:
        # Loop through actions
    ...
    self.uncertainty=np.append(self.uncertainty,tempUncertainty/(len(visited)*48),axis=1)
  ...
  def run(self):
    ...
    while (counterOut < numIterOut): #and (psat<self.Psat):
      ...
      #while self.counterIn < numIterIn:
      while self.changeCounter < numIterIn:
        if self.getStateID(self.currentPos) not in self.visited:
          self.visited.append(self.getStateID(self.currentPos))
        ...
        self.currState = self.getStateID(self.currentPos)
    ...
    #ani.save('simple_casestudy820.mp4',writer=writer,dpi=500)
    return counterOut
...
for i in np.arange(N):
    ...
    times[i]=end-start
    uncertainties.append(sg.uncertainty)
    psats.append(sg.psats[0])
    #tempPos = sg.pos
    sg.uncertainty = np.empty((1,0))
    sg.psats = np.empty((1,0))
    sg.gpT.pos=np.empty((2,0))
    sg.gpT.val=np.empty((1,0))
    sg.gpM.pos=np.empty((4,0))
    sg.gpM.val=np.empty((1,0))
    sg.gpY.pos=np.empty((2,0))
    sg.gpY.val=np.empty((1,0))
    sg.Tpos = np.empty((2,0))
    sg.Tval = np.empty((1,0))
    sg.Mpos = np.empty((4,0))
    sg.Mval = np.empty((1,0))
    sg.Ypos = np.empty((2,0))
    sg.Yval = np.empty((1,0))
...
for i in np.arange(len(uncertainties)):
            ...
            ax.set_title("Average Transition Uncertainty (Case Study 1)")
...
fig.savefig("yaw_uncertainties.pdf", bbox_inches='tight')
plt.show()
...
# %%
# Plot trajectory
img2 = plt.imread("legged_robot_simple_casestudy.png")
f2 = plt.figure()
plt.imshow(img2,extent=[0,1.5,0,1.5])
dim = np.size(sg.pos,1)
plt.xticks(np.arange(0, gridSize*stepLength+stepLength, step=stepLength))
plt.yticks(np.arange(0, gridSize*stepLength+stepLength, step=stepLength))
for i in np.arange(len(sg.stepIter)-1):
  plt.plot(sg.pos[0,sg.stepIter[i]:sg.stepIter[i+1]+1],sg.pos[1,sg.stepIter[i]:sg.stepIter[i+1]+1],alpha=0.6, marker='o', label=str(i+1))
  plt.scatter(sg.fpos[0,sg.stepIter[i]:sg.stepIter[i+1]+1],sg.fpos[1,sg.stepIter[i]:sg.stepIter[i+1]+1],c="cyan",alpha=0.4,marker='*')
plt.legend(loc='upper left', fancybox=True, title='Batch #')
plt.show()
f2.tight_layout()
f2.savefig("legged_robot_simple_casestudy_new.png", bbox_inches='tight',dpi=1000)
